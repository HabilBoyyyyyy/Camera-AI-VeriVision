...
function CustomBarTooltip({active, payload, label}) {
  ...
  return (
    <div
      ...
      <p style={{fontWeight: 700, color: "var(--clr-text)", marginBottom: 6}}>
        {label}
      </p>
      {payload.map((p, i) => (
        <div
          ...
          <span style={{color: "var(--clr-text)", fontWeight: 700}}>
            {p.value}
          </span>
        </div>
      ))}
...
export default function DashboardPage() {
    ...
    ? [
        {
          ...
          value: summary.inspections_today?.toLocaleString() ?? "0",
          icon: "qr_code_scanner",
          sub: summary.scans_change_pct || "+0.0% vs prev",
          subIcon: "trending_up",
          subOk: true,
          link: "/results",
        },
        {
          ...
          subIcon: "database",
          subOk: false,
          link: "/results?verdict=NG",
        },
        {
          label: "Active Models",
          value: summary.trained_models ?? 0,
          icon: "memory",
          sub: "All nominal",
          subIcon: "check_circle",
          subOk: false,
          link: "/ai-studio?tab=models",
        },
        {
          label: "System Uptime",
          value: summary.uptime_str || "Up 0m",
          icon: "dns",
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10 animate-fade-in space-y-6">
      {/* Command Header */}
      ...
      {/* Main bento grid: primary column + sticky ops rail */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* ============ MAIN COLUMN ============ */}
        <div className="lg:col-span-8 space-y-6">
          {/* KPI Bento Cluster */}
          <div className="grid grid-cols-2 lg:grid-cols-4 grid-rows-2 gap-4 stagger-2">
            {loading
              ? [1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className={`vv-card animate-pulse ${
                      i === 1
                        ? "col-span-2 row-span-2 h-52"
                        : i === 2
                          ? "col-span-2 h-24"
                          : "col-span-1 h-24"
                    }`}
              ...
              : statCards.map((card, i) => {
                  const spanCls =
                    i === 0
                      ...
                      : i === 1
                        ? "col-span-2"
                        : "col-span-1";
                  const clickable = card.link
                    ? "cursor-pointer hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200"
                    : "";
                  ...
                  if (i === 0) {
                    return (
                      <div
                        key={i}
                        className={`vv-card p-6 flex flex-col justify-between ${spanCls} ${clickable}`}
                        ...
                        <div
                          className="text-[12px] inline-flex w-fit items-center gap-1 px-2 py-1 rounded-full"
                          style={{
                            color: card.subOk
                              ? "var(--clr-accent)"
                              : "var(--clr-text-sub)",
                  ...
                  if (i === 1) {
                    return (
                      <div
                        key={i}
                        className={`vv-card p-5 flex items-center gap-4 ${spanCls} ${clickable}`}
                        onClick={() => card.link && router.push(card.link)}>
                        <span
                        ...
                        <div
                          className="ml-auto text-[11px] flex items-center gap-1 shrink-0"
                          style={{
                            color: card.subOk
                              ? "var(--clr-accent)"
                              : "var(--clr-text-sub)",
                  ...
                  return (
                    <div
                      key={i}
                      className={`vv-card p-4 flex flex-col justify-between gap-2 ${spanCls} ${clickable}`}
                      onClick={() => card.link && router.push(card.link)}>
                      <span
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 stagger-4">
            {/* Donut */}
            <div className="lg:col-span-5 vv-card p-5 flex flex-col">
            ...
            {/* Bar Chart */}
            <div className="lg:col-span-7 vv-card p-5 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3
                    className="text-sm font-semibold uppercase tracking-wider"
          ...
          {/* Recent Fails Filmstrip */}
          {summary?.recent_ng_parts?.length > 0 ? (
            <div className="vv-card overflow-hidden stagger-5">
              <div
                ...
                <div>
                  ...
                  <p
                    className="text-xs mt-0.5"
                    style={{color: "var(--clr-text-sub)"}}>
                    Latest NG parts detected
                  </p>
                </div>
                <Link
                  href="/results?verdict=NG"
              ...
              <div className="p-4 flex gap-3 overflow-x-auto">
                {summary.recent_ng_parts.slice(0, 8).map((ng, i) => (
                  <Link key={i} href={`/results?search_id=${ng.id}`}>
                    <div className="relative w-28 h-28 shrink-0 rounded-lg overflow-hidden border border-[#2b313a] group cursor-pointer bg-[#14171c]">
                      {ng.image_path ? (
                        <img
                          src={`${BASE_URL}${ng.image_path}`}
                          alt="NG Part"
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                        />
                      ) : (
                        <div className="w-full h-full flex flex-col items-center justify-center">
                          <span className="material-symbols-outlined text-[28px] text-[#3a4149]">
                            image_not_supported
                          </span>
                        </div>
                      )}
            ...
            <div
              className="vv-card overflow-hidden flex flex-col items-center justify-center p-6 text-center text-xs stagger-5"
              style={{color: "var(--clr-text-muted)"}}>
          ...
          <div className="vv-card overflow-hidden stagger-6">
            <div
              className="p-4 flex justify-between items-center"
              style={{
                borderBottom: "1px solid var(--clr-border)",
                  ...
                  {(summary?.top_models?.length > 0
                    ...
                    <tr
                      ...
                      <td className="px-5 py-3">
                        <span
                          className={`badge ${row.status === "Deployed" ? "badge-deployed" : "badge-archived"}`}>
                          {row.status}
                        </span>
                      </td>
                    </tr>
        ...
        <aside className="lg:col-span-4 space-y-5 lg:sticky lg:top-6 self-start">
          ...
          {!loading && alertsData?.summary && (
            <div
              ...
              style={{
                borderColor: "rgba(0,140,199,0.2)",
                background: "rgba(0,140,199,0.03)",
              }}>
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                ...
                <button
                  ...
                  <span
                    className={`material-symbols-outlined text-[14px] ${refreshingInsight ? "animate-spin" : ""}`}>
                    refresh
                  </span>
          ...
          {!loading && (
            <div className="vv-card flex flex-col overflow-hidden">
              <div
                className="p-4 shrink-0 flex justify-between items-center flex-wrap gap-2"
                style={{
                  borderBottom: "1px solid var(--clr-border)",
                  background: "var(--clr-surface-low)",
                }}>
              ...
              <div
                className="flex-1 divide-y max-h-[640px] overflow-y-auto"
                style={{divideColor: "var(--clr-border)"}}>
                {displayedAlerts.length > 0 ? (
                  displayedAlerts.map((alert, i) => {
                    const cfg =
                      SEVERITY_CFG[alert.severity] || SEVERITY_CFG.info;
                    ...
                    return (
                      <div
                        key={alert.id || i}
                        className="p-4 flex flex-col gap-3 transition-colors relative hover:bg-[var(--clr-surface-low)]"
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className={`badge ${cfg.badgeCls}`}>
                                {cfg.label}
                                {cfg.pulse && (
                                  <span className="ml-1 inline-block w-1.5 h-1.5 rounded-full bg-red-500 animate-ping" />
                                )}
                              </span>
                              {timeStr && (
                                <span
                        ...
                        <button
                          ...
                          className="text-xs btn-outline w-full justify-center px-2 py-1 flex items-center gap-1 opacity-60 hover:opacity-100">
                          <span className="material-symbols-outlined text-[14px]">
                            check
                          </span>
                          Acknowledge
