...
function TrophyIcon(p) {
  return <FontAwesomeIcon icon={faTrophy} className={p.className || ""} />;
}
...
function CustomPieTooltip({active, payload}) {
  ...
  return (
    <div className="bg-[#1a1e24] border border-[#333b46] rounded px-3 py-2 text-xs shadow-2xl">
      <span style={{color: payload[0].payload.fill}} className="font-bold">
        {payload[0].name}:{" "}
      </span>
...
export default function DashboardPage() {
    ...
    ? [
        {
          ...
          desc: "Upload new training data",
        },
        {
          href: "/training",
          label: "Train Model",
          icon: <CubeIcon className="w-5 h-5" />,
          desc: "Configure and start training",
        },
        {
          href: "/live",
          label: "Live Inspection",
          icon: <CameraIcon className="w-5 h-5" />,
          desc: "Inspect parts in real-time",
        },
      ]
    : [
        {
        ...
        },
        {
          ...
          desc: "Browse inspection history",
        },
      ];
  ...
  const yieldData = summary?.today_yield
    ? [
        ...
        {
          name: "Uncertain",
          value: summary.today_yield.Uncertain,
          fill: "#f5a623",
        },
      ].filter((d) => d.value > 0)
    : [];
  ...
  const totalToday = summary?.today_yield
    ? summary.today_yield.OK +
      summary.today_yield.NG +
      summary.today_yield.Uncertain
    : 0;
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="animate-fade-in stagger-1">
        ...
        <p className="text-sm text-[#5a6270] ml-8">
          Welcome back,{" "}
          <span className="text-[#8a93a3] font-semibold">{user?.username}</span>
          <span className="ml-2 text-[#3a4149]">·</span>
          <span className="ml-2 text-[#3a4149]">
          ...
          {statCards.map((card, i) => (
            <div
              ...
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-[10px] font-bold text-[#5a6270] uppercase tracking-widest mb-2 font-mono">
                    {card.label}
                  </p>
                  <p className="text-4xl font-bold text-[#e4e7eb] font-mono">
                    {card.value}
                  </p>
                </div>
                <div
                  className={`${card.iconColor} opacity-60 group-hover:opacity-100 transition-opacity`}>
                  {card.icon}
                </div>
              </div>
              <div
          ...
          {hasAnyInspections ? (
                ...
                {[
                  {
                    label: "OK",
                    color: "bg-[#2fb380]",
                    val: summary?.today_yield?.OK ?? 0,
                  },
                  {
                    label: "NG",
                    color: "bg-[#e5484d]",
                    val: summary?.today_yield?.NG ?? 0,
                  },
                  {
                    label: "?",
                    color: "bg-[#f5a623]",
                    val: summary?.today_yield?.Uncertain ?? 0,
                  },
                ].map(({label, color, val}) => (
                  <div key={label} className="flex items-center gap-1.5">
                    <span className={`w-2 h-2 rounded-sm ${color}`} />
                    <span className="text-xs text-[#8a93a3]">{label}</span>
                    <span className="text-xs font-bold text-[#dbe0e6] font-mono">
                      {val}
                    </span>
                  </div>
                ))}
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <Link
                  href="/live"
                  className="text-xs text-[#f5a623] hover:text-[#ffc157] mt-1 inline-block font-semibold transition-colors">
                  Start Inspection →
                </Link>
              </div>
            </div>
            ...
            <ResponsiveContainer width="100%" height={210}>
              <BarChart
                ...
                <Tooltip
                  ...
                  cursor={{fill: "rgba(138,147,163,0.06)"}}
                />
                <Bar
                  dataKey="OK"
                  stackId="a"
        ...
        <div className="lg:col-span-2 glass-card overflow-hidden">
          <div className="px-6 py-4 border-b border-[#2b313a] bg-[#181c22] flex items-center justify-between">
            ...
            <Link
              ...
              className="text-xs text-[#f5a623] hover:text-[#ffc157] font-semibold transition-colors">
              View All →
            </Link>
          </div>
          ...
          {summary?.recent_ng_parts?.length > 0 ? (
            <div className="p-5 grid grid-cols-4 gap-3">
              {summary.recent_ng_parts.map((part, i) => (
                <div
                  key={part.id}
                  className="group relative rounded overflow-hidden border border-[#e5484d]/25 bg-[#0f1216] aspect-square">
                  {part.image_path ? (
                    <img
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2">
                    <span className="text-[9px] font-mono text-[#f26e72] font-bold">
                      {(part.confidence * 100).toFixed(0)}% conf
                    </span>
                    <span className="text-[9px] text-[#8a93a3] font-mono">
                      {part.id.substring(0, 8)}
                    </span>
                  </div>
                  {/* NG badge */}
                  <div className="absolute top-1.5 right-1.5 bg-[#e5484d] text-white text-[8px] font-bold px-1.5 py-0.5 rounded-sm font-mono">
                    NG
                  </div>
                </div>
              ))}
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-xs text-[#5a6270] mt-1">
                All recent parts passed inspection
              </p>
            </div>
          )}
        </div>
            ...
            {summary?.top_models?.length > 0 ? (
              <div className="divide-y divide-[#232830]">
                {summary.top_models.map((m, i) => (
                  <div
                    key={m.id}
                    className="px-5 py-3 flex items-center gap-3 hover:bg-[#1e232a] transition-colors">
                    <span
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-[#dbe0e6] truncate">
                        {m.name}{" "}
                        <span className="text-[#5a6270]">v{m.version}</span>
                      </p>
                    ...
                    {m.score !== null ? (
                      ...
                      <span className="text-[10px] text-[#3a4149] font-mono">
                        —
                      </span>
                    )}
                  </div>
            ...
            <div className="divide-y divide-[#232830]">
              {quickActions.map((action, i) => (
                <Link
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-[#dbe0e6]">
                      {action.label}
                    </p>
