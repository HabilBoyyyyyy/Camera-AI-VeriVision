...
function ImageIcon(p) {
  return <FontAwesomeIcon icon={faImage} className={p.className || ""} />;
}
function ArrowLeftIcon(p) {
  return <FontAwesomeIcon icon={faArrowLeft} className={p.className || ""} />;
}
...
export default function DatasetExplorer({datasetId, onUpdate}) {
  ...
  if (loading) {
    return (
      <div className="p-6 text-center text-xs text-[#5a6270] animate-pulse">
        Loading files...
      </div>
    );
  }
    ...
    <div className="mt-4 border border-[#2b313a] rounded overflow-hidden bg-[#0f1216]">
      ...
      <div className="p-4 max-h-96 overflow-y-auto">
        {folderList.length === 0 && currentFiles.length === 0 ? (
          <div className="text-center py-8 text-xs text-[#5a6270]">
            Empty directory
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
            {folderList.map((f) => (
              <div
                ...
                <span className="text-[10px] font-bold text-[#8a93a3] text-center truncate w-full">
                  {f}
                </span>
              </div>
            ))}
            ...
            {currentFiles.map((file, idx) => (
              <div
                key={idx}
                className="group flex flex-col items-center rounded overflow-hidden border border-[#2b313a] bg-[#1a1e24] relative">
                {/* Delete Button */}
                <button
                ...
                <div className="w-full aspect-square bg-[#0f1216] relative">
                  ...
                  <a
                    ...
                    <span className="px-2 py-1 rounded bg-[#f5a623] text-[#14171c] text-[10px] font-bold shadow-lg">
                      View
                    </span>
                  </a>
                </div>
